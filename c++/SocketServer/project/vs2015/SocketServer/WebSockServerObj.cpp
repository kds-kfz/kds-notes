	...
	WEB_NOTIFY_PROC p_webHandle, unsigned int p_uiThreadNum, unsigned int p_uiQueueNum, char *p_szErr, const char *p_szLogFold)
{
	...
	if (!g_CWebPackServer->Start(p_szIp, p_unPort))
	{
		_snprintf(p_szErr, 1024, "code=%d,msg=%s",
			g_CWebPackServer->GetLastError(), g_CWebPackServer->GetLastErrorDesc());
		DeleteWebObj();
		return false;
	}
...
bool CWebSockServerObj::CreateWssSock(const char *p_szIp, unsigned short p_unPort, unsigned int p_uiRBufLen, unsigned int p_uiMaxConnectNum, unsigned int p_uiMaxAcceptNum,
	WEB_NOTIFY_PROC p_wssHandle, unsigned int p_uiThreadNum, unsigned int p_uiQueueNum, char *p_szErr,
	const char *p_szPemCertFile, const char *p_szPemKeyFile,
	const char *p_szKeyPassword, const char *p_szCAPemCertFileOrPath,
	const char *p_szLogFold)
{
	// wyl 2026-03-30：当前版本尚未实现 WSS 建链和证书装载，必须明确返回失败，避免上层误判服务已启动成功。
	if (nullptr != p_szErr)
	{
