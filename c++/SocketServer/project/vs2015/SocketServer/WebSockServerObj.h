...
public:
	...
	virtual bool CreateTcpSock(const char *p_szIp, unsigned short p_unPort, unsigned int p_uiRBufLen, unsigned int p_uiMaxConnectNum, unsigned int p_uiMaxAcceptNum,
		TCP_NOTIFY_PROC p_tcpHandle, unsigned int p_uiThreadNum, unsigned int p_uiQueueNum, char *p_szErr, const char *p_szLogFold = nullptr) { return false; }
	// 关闭服务
	virtual void StopTcpSock() { return; }
	// 发送应答
	virtual void TcpSockSend(void *p_refServer, void *p_refClient, const char *p_szData, int p_iDataLen) { return; }
	// 关闭客户连接
	virtual void TcpSockClose(void *p_refServer, void *p_refClient, const char *p_szData, int p_iDataLen) { return; }
	// 比较彼此客户端是否一致;
	virtual int TcpSockCompare(void *p_refSrcClient, void *p_refObjClient) { return -1; }
	...
	// 释放请求
	virtual bool DelHttpAsynReq(unsigned long long p_lluReqId) { return false; }
	// 关闭服务
	virtual void StopHttpSock() {}
