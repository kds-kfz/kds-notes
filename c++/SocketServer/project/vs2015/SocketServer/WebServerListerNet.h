...
{
	//开始解析通知
	EnHttpParseResult OnMessageBegin(IHttpServer*, CONNID) { return HPR_OK; }
	...
	//BODY 报文通知
	EnHttpParseResult OnBody(IHttpServer*, CONNID, const BYTE*, int)
	{
		return HPR_OK;
	}
	//状态行解析完成通知（仅用于 HTTP 客户端）
	EnHttpParseResult OnStatusLine(IHttpServer*, CONNID, USHORT, LPCSTR) { return HPR_OK; }
	//请求头通知
	...
	//Chunked 报文头通知
	EnHttpParseResult OnChunkHeader(IHttpServer*, CONNID, int) { return HPR_OK; }
	//Chunked 报文结束通知
	EnHttpParseResult OnChunkComplete(IHttpServer*, CONNID) { return HPR_OK; }
	//完成解析通知
	EnHttpParseResult OnMessageComplete(IHttpServer*, CONNID)
	{
		return HPR_OK;
	}
	//升级协议通知
	...
	//解析错误通知
	EnHttpParseResult OnParseError(IHttpServer*, CONNID, int, LPCSTR) { return HPR_OK; }
	//WebSocket 数据包头通知
	...
	//数据到达通知（PUSH 模型）
	EnHandleResult OnReceive(ITcpServer*, CONNID, const BYTE*, int) { return HR_OK; }
	//数据到达通知（PULL 模型）
