...
// 集中推送线程使用的包节点，持有自描述载荷副本并由消费方负责校验解压。
struct ST_PACK_QUEUE 
{
	...
	long long lReqNo;
	// Ice 回调深拷贝的逐包载荷，回调返回后仍保持有效。
	::JSONBINRPC::BinaryPayload stPayload;
	// 接收连接配置的协议版本，用于工作线程拒绝不兼容载荷。
	int iProtocolVersion;
	// 接收连接允许的原始载荷上限，防止解压放大内存占用。
...
// DLL 对外 HANDLE 的真实结构，统一包装服务实例或具体客户端代理。
	struct ST_JSON_BIN_HANDLE
	{
		...
		std::map<DWORD,int>			mapSubId;		// 订阅信息：值 1 表示订阅，键 0 表示接收全部通知。
		// 主动向下游推送，管理的client信息
...
// Ice 3.5 AMD 注册入口，迁移期保留给旧代码路径参考。
	...
	// Ice 3.8 二进制 RPC 入口，请求进入独立有界队列后由工作线程处理。
	...
	// Ice 3.8 二进制 PUT 入口，和 BinaryRPC 共用压缩、限流和应答流程。
...
// 停止 Ice、SocketServer 和后台线程，释放 DLL 内部资源。
	...
	// 返回最近一次初始化或 Binary 编解码失败详情，供 DLL 边界转换为统一错误。
	...
	// 异步调用保持 BinaryPayload 编码状态并把结果所有权交给调用方。
...
protected:
	...
	// 移动 Ice BinaryResponse 的 BinaryPayload 所有权，不解压也不复制正文。
...
// 旧 TCP/UDP 快速通道的注册协商状态；Ice BinaryPayload 不依赖该连接级开关。
	bool											m_bSnappy;
	bool											m_bRegisterOk;
...
// 是否等待异步 RPC 完成，默认保留旧版阻塞行为，XML 可配置为 0 提升吞吐。
	...
	int											m_iBinaryProtocolVersion;		// 二进制传输协议版本。
	int											m_iBinaryCompressionThresholdBytes;	// Snappy 最小压缩字节数。
	int											m_iBinaryMaxPayloadBytes;		// 单次请求或应答允许的最大原始字节数。
	int											m_iBinaryCallTimeoutMs;		// 二进制同步调用超时毫秒数。
	int											m_iBinaryServerWorkerThreads;	// 服务端二进制工作线程数。
	int											m_iBinaryServerQueueCapacity;	// 服务端二进制等待队列容量。
	int											m_iBinaryMaxPendingAsync;		// 客户端允许的最大异步积压数。
	std::atomic<long long>						m_lBinaryPendingAsync;		// 当前客户端二进制异步积压数。
	std::atomic<long long>						m_lBinaryPendingResponses;	// 当前服务实例尚未移除的服务端应答句柄数。
	std::atomic<bool>							m_bBinaryWorkerStop;		// 二进制工作线程退出标记。
	std::mutex								m_clBinaryQueueMutex;		// 二进制服务端队列互斥锁。
	std::condition_variable					m_clBinaryQueueCondition;	// 二进制服务端队列唤醒条件。
	std::deque<std::shared_ptr<ST_BINARY_SERVER_TASK>> m_dequeBinaryTasks;	// 待处理普通二进制请求队列。
	std::deque<std::shared_ptr<ST_BINARY_SERVER_TASK>> m_dequeBinaryControlTasks; // 优先处理的控制请求队列。
	std::vector<std::thread>					m_vecBinaryWorkers;		// 二进制服务端工作线程集合。
	int										m_iBinaryResponseWorkerThreads; // Snappy 编码和 Ice 回调线程数。
	int										m_iBinaryResponseQueueCapacity; // 已完成结果的应答队列容量。
	int										m_iBinaryReclaimWorkerThreads; // 最终释放请求和应答资源的线程数。
	std::atomic<bool>						m_bBinaryResponseWorkerStop; // 应答线程退出标记。
	std::mutex							m_clBinaryResponseQueueMutex; // 已完成结果队列锁。
	std::condition_variable					m_clBinaryResponseQueueCondition; // 应答队列唤醒条件。
	std::deque<ST_BINARY_RESPONSE_CONTEXT*>	m_dequeBinaryResponses; // 直绑定上下文应答任务。
	std::vector<std::thread>					m_vecBinaryResponseWorkers; // 应答线程集合。
	std::atomic<bool>						m_bBinaryReclaimWorkerStop; // 回收线程退出标记。
	std::mutex							m_clBinaryReclaimQueueMutex; // 最终释放队列锁。
	std::condition_variable					m_clBinaryReclaimQueueCondition; // 回收队列唤醒条件。
	std::deque<ST_BINARY_RESPONSE_CONTEXT*>	m_dequeBinaryReclaims; // 等待最终释放的上下文。
	std::vector<std::thread>					m_vecBinaryReclaimWorkers; // 默认两个回收线程。
	std::mutex							m_clBinaryPendingContextMutex; // 只用于容量和停机收敛。
	std::set<ST_BINARY_RESPONSE_CONTEXT*>	m_setBinaryPendingContexts; // 尚未转移到应答队列的上下文。
...
public:
	...
	void								*			m_pServerParam;
	func_IceBinaryServerCallback					m_pfnBinaryServerCallback;	// 二进制服务端业务回调。
	func_IceBinaryServerCallbackEx				m_pfnBinaryServerCallbackEx; // 支持延迟应答的回调。
	func_IceBinaryPriorityClassifier			m_pfnBinaryPriorityClassifier; // 控制请求分类器。
	void								*			m_pBinaryServerParam;		// 二进制回调透传参数。
};
