...
public:
	CDataQueue()
	{
		InitializeCriticalSection(&m_csLockLockSyn);
	...
	virtual ~CDataQueue()
	{
		DeleteCriticalSection(&m_csLockLockSyn);
	}
...
private:
// 保护 list 和缓存队列长度的临界区。
	CRITICAL_SECTION	m_csLockLockSyn;
...
// 暴露手工加锁入口，供需要遍历 Reffer 的旧代码保持互斥。
	void		RefLock()
	{
		EnterCriticalSection(&m_csLockLockSyn);
	}
	void		RefUnlock()
	{
		LeaveCriticalSection(&m_csLockLockSyn);
	}
...
// 连续块内存首地址，由 Init 分配并由 Release 释放。
    unsigned char *pData;
    unsigned char chFirstAvailableBlock,chBlocksAvailable;
