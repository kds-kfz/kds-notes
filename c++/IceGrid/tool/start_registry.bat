...
set "SERVICE_NAME=icegridregistry.MtIceGrid"
REM 初始化统一失败说明。
set "FAIL_MESSAGE="

REM 检查 Registry Windows 服务是否已经安装。
sc query "%SERVICE_NAME%" >nul 2>&1
REM 服务不存在时提示先执行总安装脚本。
if errorlevel 1 (
	REM 保存服务未安装说明。
	set "FAIL_MESSAGE=Registry 服务尚未安装，请先运行 tool\install_icegrid_wtgrid.bat。"
	REM 跳转统一失败出口。
	goto fail
)
...
REM 显示即将启动的服务名。
echo [INFO] 启动 Registry 服务 %SERVICE_NAME%...
REM 请求 Windows 服务控制管理器启动 Registry。
...
if errorlevel 1 (
	...
	if errorlevel 1 (
		REM 保存启动命令失败说明。
		set "FAIL_MESSAGE=Registry 服务启动命令失败。"
		REM 跳转统一失败出口。
		goto fail
	)
)
...
if errorlevel 1 (
	REM 保存服务状态超时说明。
	set "FAIL_MESSAGE=Registry 服务未在 30 秒内进入 Running 状态。"
	REM 跳转统一失败出口。
	goto fail
)

REM Registry 已运行后的管理接口健康检查入口。
:health_check
REM 最多等待 30 秒确认 icegridadmin 可访问 Registry。
call :wait_registry
REM 健康检查失败时进入统一错误出口。
if errorlevel 1 goto fail
REM 显示 Registry 后台运行成功。
echo [SUCCESS] Registry 服务已在后台运行。
REM 用成功错误码结束脚本。
exit /b 0

REM 判断 Registry 是否处于 Running 的子程序。
:is_running
...
echo [INFO] 错误日志：%CD%\log\registry_stderr.txt
...
:invalid_arguments
REM 打印允许的调用方式。
echo [ERROR] 用法：start_registry.bat [--nopause]
REM 默认暂停以便双击运行时查看错误。
pause
REM 使用参数错误返回码结束脚本。
exit /b 2
...
:show_help
REM 打印脚本功能。
echo 用途：启动已安装的 MtIceGrid Registry Windows 服务并检查管理接口。
REM 打印调用格式。
echo 用法：start_registry.bat [--nopause]
REM 打印人工调用示例。
echo 示例：start_registry.bat
REM 打印自动化调用示例。
echo 示例：start_registry.bat --nopause
REM 说明前置条件。
echo 前置条件：已运行 install_icegrid_wtgrid.bat 安装 Registry 服务。
REM 说明管理员权限规则。
