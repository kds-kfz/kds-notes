...
//2017/10/24
//运算符重载
    friend ostream &operator<<(ostream &u,const String &s);
...
//    friend String &operator=(const String s);
    ...
    friend void resize(String &s,int n);
#endif
};

//仿造std::string的用法，定义一个String
//构造函数，拷贝，析构，重载运算符
//+,=,+=,==,!=,<,>,<=,>=,[],>>,<<
//replace,erase ...
...
//2017/10/24
//运算符重载
ostream &operator<<(ostream &u,const String &s){
...
String operator+(const String &s1,const String &s2){
    ...
    String news;
    delete []news.ptr;
    news.ptr=new char[s1.len+s2.len]{};
    memcpy(news.ptr,s1.ptr,s1.len);
    memcpy(news.ptr+s1.len,s2.ptr,s2.len+1);
...
bool operator!=(const String &s1,const String &s2){
    cout<<"类外!="<<endl;
    return !(s1.ptr==s2.ptr)&&s1.len!=s2.len;
}
bool operator<(const String &s1,const String &s2){
    cout<<"类外<"<<endl;
    ...
    while(i<n){
	if(s1.ptr[i]!=s2.ptr[i]){
	    if(s1.ptr[i]<s2.ptr[i]&&s1.ptr[i]!='\0')
		return true;
	}
	i++;
    }
...
bool operator>(const String &s1,const String &s2){
    ...
    while(i<n){
	if(s1.ptr[i]!=s2.ptr[i]){
	    if(s1.ptr[i]>s2.ptr[i]&&s2.ptr[i]!='\0')
		return true;
	}
	i++;
    }
