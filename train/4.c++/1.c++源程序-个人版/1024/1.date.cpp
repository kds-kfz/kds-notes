...
struct Date{
    ...
    int year,month,day;
    public:
    //在类内声明一个函数是类的友元函数，这个函数可以访问类内的私有私有成员
...
int main(){
    ...
    /*
    Date d1;
    //oprator(oprator<<(cout,d1),endl)
    cout<<d1<<endl;

    std::cout 是一个输出对象ostream类型
    <<输出运算符
    d1是一个对象Date类型
    ...
    */
    ...
    //d1.operator+(d2)，调用类内的+
    cout<<d1.operator+(d2)<<endl;
    //operator+(d1,d2)，调用类外的+
    cout<<operator+(d1,d2)<<endl;
    //d1+d2，可以调用类内的+，也可以调用类外的+，
    //如果类内和类外同时存在+，优先调用类内的+
    cout<<d1+d2<<endl;
    ...
    //d1是非const，const的函数也是一个接近的函数
    //d1==d2，找接近的类型，d1不是const，
    //所以两种版本都可调用，会有二义性
