...
class operate_Sqlite3{
	...
	sqlite.exec("select *from goods");	
	int col=sqlite.get_c();
	int row=sqlite.get_r();
	Factory_All ap;//定义临时工厂
//	Armor *p=NULL; //定义临时道具指针
	string name;
	string nature;
	int num1[18]={0};
	float num2[2]={0.0};
	for(int i=col;i<(row+1)*col;i++){//从第1行开始
//	    sqlite[i][j];  //第1行第0列
	    chTost(name,sqlite[i][5]);//ch字符转换成string
	    chTost(nature,sqlite[i][6]);
	    num1[0]=*sqlite[i][0];num1[1]=*sqlite[i][1];num1[2]=*sqlite[i][2];
	    num1[3]=*sqlite[i][3];num1[4]=*sqlite[i][4];num1[5]=*sqlite[i][7];
	    num1[6]=*sqlite[i][8];num1[7]=*sqlite[i][11];num1[8]=*sqlite[i][12];
	    num1[9]=*sqlite[i][13];num1[10]=*sqlite[i][14];num1[11]=*sqlite[i][15];
	    num1[12]=*sqlite[i][16];num1[13]=*sqlite[i][17];num1[14]=*sqlite[i][18];
	    num1[15]=*sqlite[i][19];num1[16]=*sqlite[i][20];num1[17]=*sqlite[i][21];
	    num2[0]=*sqlite[i][9];num2[1]=*sqlite[i][10];
    /*
	    Armor *p=ap.produceArm(
	    num1[0],num1[1],num1[2],num1[3],num1[4],name,nature,
	    num1[5],num1[6],num1[7],num1[8],num2[0],num2[1],num1[9],num1[10],
	    num1[7],num1[11],num1[12],num1[13],num1[14],num1[15],num1[16]
	    );
	    */
	    Armor *p1=ap.produceArm(
	    1001,10,15,0,0,"限定头盔","增加攻击物抗和魔抗",
	    0,0,0,0,60,0,100,40,2000,4000,30,0,0,1);
	    Goods.insert({p1->getid(),{p1,num1[17]}});
//	    if((i+1)%col==0)
//		j=0;	    //重置列数
