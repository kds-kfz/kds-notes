...
/******************************************************************/
//输入一串字符串，如数正确返回0,错误返回1
int my_input3(string &s,int n){
    s.resize(0);
    int i=0;
    int j=0;
    show_cursor();//显示光标
...
//输入一串字符，输入错误则光标在原位置，不刷屏
void fixed_my_input(string *s,int n,int &flag){
    save_cursor();
    int count=0;
    flag=1;
    while(flag){
...
//返回1个字符，用于界面选择，输入错误则光标在原位置，不刷屏
char fixed_my_input1(string *s,int n,int &flag){
    save_cursor();
    int count=0;
    int num=0;
    flag=1;
...
void display_cheek1(int x,int y,int right){
    int i,j;
    for(i=0;i<x;i++){
	if(i==0||i==x-1){
	    //打印前移动光标位置
...
void draw_row_line1(int up,int right,int n,const char type[]){
    fun_cursor_up(up);
    fun_cursor_right(right);
    for(int i=0;i<n;i++)
	cout<<type;
}
void draw_row_line2(int down,int left,int n,const char type[]){
    fun_cursor_down(down);
    fun_cursor_left(left);
    for(int i=0;i<n;i++)
	cout<<type;
}
void draw_col_line1(int up,int left,int right,int n,const char type[]){
    fun_cursor_up(up);
    fun_cursor_left(left);
    fun_cursor_right(right);
//    save_cursor();
...
void draw_col_line2(int up,int right,int n,const char type[]){
    fun_cursor_up(up);
    fun_cursor_right(right);
//    fun_cursor_left(left);
//    save_cursor();
...
//定坐标，输入一个数
void set_point_num(int x,int y,int num){
    set_cursor(1,1);
    set_cursor(x,y);
    cout<<red<<num;
}
...
// 清屏+画边框+光标上移n行
void interface(int x,int y,int n){
    system("clear");//清屏
