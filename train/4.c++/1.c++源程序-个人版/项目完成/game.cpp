...
class operate_Sqlite3{
	...
	delete_ok=sqlite.exec("drop table parts;");
	if(delete_ok!=SQLITE_OK)
	    cout<<"删除parts表格失败"<<endl;
	
	delete_ok=sqlite.exec("drop table rares;");
	if(delete_ok!=SQLITE_OK)
	    cout<<"删除rares表格失败"<<endl;
    }
    /*********************************************************************/
    //创建goods表格
...
void Game::Register(){
	...
	int maxid=3000;
	for(auto m:PLAYER)
	    maxid=Max(maxid,m.second->getid());
	maxid+=1;
	
	Player *pl=new Player{maxid,name1,key1};//创建新玩家
...
//id+等级+重量+时装+部位+名字+描述+0+0+0+0+物伤+0+物抗+魔抗+耐久+造价+稀有+价值+状态
    ...
    Armor *p2=ap.produceArm(
	    ...
	    100,0,20.0,0,0,0,60,0,1800,4500,35,Epic,Gold,1);
    Armor *p3=ap.produceArm(
    ...
    Armor *p7=ap.produceArm(
	    ...
	    100,0,35,0,0,0,60,0,1800,4500,35,Senior,Gold,2);
    Armor *p8=ap.produceArm(
