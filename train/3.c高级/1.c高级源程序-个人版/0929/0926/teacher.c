...
void te_news(ST1 *te_head){
    ...
    printf(cursor_right12"帐号\t密码\t工号\t姓名\t年龄\t性别\n\n");
    printf(cursor_right12"%s\t%s\t%d\t%s\t%d\t%s\n",
...
void display_all_student(){
	...
	printf(cursor_right12"帐号\t学号\t姓名\t年龄\t性别\t成绩\n\n");
...
void record_mark(){
    ...
    int st_id=0;
    int fg=0;
    printf(cursor_right4"教 师 录 成 绩 界 面\n\n");
	...
	if(st_head){
	    ...
	    float mark=0;
	    printf(cursor_right2"学生：%s\n",st_head->value.name);
	    printf(cursor_right5"请输入学生成绩:");
	    scanf("%f",&mark);
	    st_head->value.score=mark;
	    save_link(head);
	    
	    int count=0;
	    printf(cursor_right6"是否继续录成绩(y or n)\n\n");
	    while(1){
	    char choose=0;
	    getchar();
