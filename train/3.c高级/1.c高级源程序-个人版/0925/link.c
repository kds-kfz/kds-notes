...
//函数14
ST *exchange_link(ST *head,int d1,int d2){//链表数据交换，未完成
    ST *p=head;
    ST *arr[len_link(head)];
    return NULL;
}
//函数15，文件以文本形式读取数据，并创链表
ST *read_link(){
	...
	ST *head=NULL;
	while(1){
	    ret=fscanf(fp,"%d %s %c %d",&d1.num,d1.name,&d1.sex,&d1.age);
	    if(ret==4)
...
//函数18，数据以二进制形式，并创链表
void save1_link(ST *head){
    FILE *fp=fopen("student1.txt","wb");
    while(head){
	fwrite(&head->value,sizeof(Data),1,fp);
	head=head->next;
    }
    printf("二进制数据保存成功\n");
    fclose(fp);
