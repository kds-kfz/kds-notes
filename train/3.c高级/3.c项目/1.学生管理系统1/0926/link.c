...
void printf_link_pre(ST *head){//从头打印
    ...
    head->value.age,head->value.sex,head->value.score);
    head=head->next;
    }
}
//函数4(教师)
void printf1_link_pre(ST1 *head){//从头打印
    ...
    printf("\033[20C");
    printf("%s\t%d\t%s\t%d\t%s\n",
    head->value1.number,head->value1.num,
    head->value1.name,head->value1.age,head->value1.sex);
    head=head->next;
    }
}
//函数5
...
ST *select_sort_link(ST *head){//选择排序
	...
	for(p=head;p->next!=NULL;p=p->next){
	    if(p->next->value.score > pmax->value.score)
		pmax=p->next;
	}
...
ST *read_link(){
	...
	ST *head=NULL;
	while(1){
	    ret=fscanf(fp,"%s %s %d %s %d %s %f",
		d1.number,d1.key,&d1.num,d1.name,
		&d1.age,d1.sex,&d1.score);
	    if(ret==7)
