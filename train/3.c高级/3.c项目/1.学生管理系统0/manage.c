...
//	getchar();
	...
	if(!strcmp(key1,key)){//校验旧密码成功
	    printf(cursor_right4"密码校验成功\n\n");
//	    putchar(10);
	    getchar();
...
//	printf(cursor_right2"数据不存在\n\n");
	printf(cursor_right2"需要教师完成注册,谢谢\n");
	sleep(2);
	return;
    }
    else{
	...
	count+=1;
	printf(cursor_right3"所 有 教 师 信 息\n");
	printf(cursor_right2"教 师 总 人 数：%d人\n\n",count);
//	putchar(10);
...
void del_student(){
    ...
    if(head==NULL){
	printf(cursor_right2"需要学生完成注册,谢谢\n");
	sleep(2);
	clear_buf();
	return;
    }
...
void del_teacher(){
    ...
    if(head==NULL){
	printf(cursor_right1"需要教师完成注册,谢谢\n");
	sleep(2);
	clear_buf();
	return;
    }
...
void take_clas(){
	...
	if(choose=='1'){
	    ...
	    printf(cursor_right6"是否确认目前操作?(y or other):");
	    getchar();
	    //scanf("%c",&choose);
	    choose=getchar();
	    if(choose=='y'){
	    ...
	    printf(cursor_right2"一键分班成功\n");
	    sleep(2);
	    clear_buf();
	    }
	    else{
		printf(cursor_right2"一键分班失败\n");
		sleep(2);
		clear_buf();
		break;
	    }
