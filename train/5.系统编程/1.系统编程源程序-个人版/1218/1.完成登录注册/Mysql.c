...
void create_all_table(Mysql_t *m){
    ...
    char *sql5="create table filmseat(number int,seat varchar(50))";
...
void insert_filmseat(Mysql_t *m,Film_t *p){
    //增加前先删掉该条信息
    char delete_buf[1024];
    sprintf(delete_buf,"delete from filmseat where number=%d;",p->c_number);
    Mysql_query(m,delete_buf);
    ...
    buf[i]='\0';
    char sql_buf[1024];
    sprintf(sql_buf,"insert into filmseat(number,seat) values(%d,'%s');",
    p->c_number,buf);
    Mysql_query(m,"set names utf8");//设置支持插入中文
    Mysql_query(m,sql_buf);
}
...
void insert_all_data(Mysql_t *m){
    ...
    Movie pe8={4004,"变形金刚4",3001,"乐巢影院","120分钟","2017-12-18",89,40,1};

    Film_t pe9={3001,{0},{0}};
    Film_t pe10={3002,{0},{0}};
