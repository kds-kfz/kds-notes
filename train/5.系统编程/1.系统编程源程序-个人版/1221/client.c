...
void change_cinema(int fd){
	...
	if(choose=='0')
	    ...
	    int res=read_server_data(fd,NULL);//重写读取影院函数
	    if(res==1){
...
void edit_movie(int fd){
	...
	set_point_ch(33,9,"3.删除电影");
	set_point_ch(33,11,"4.返回");
	set_point_ch(33,13,"请输入选择:");
	char choose=fixed_input1(buf,20,&flag);
	switch(choose){
	    case '1':break;
