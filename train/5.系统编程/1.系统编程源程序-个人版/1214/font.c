...
//在指定坐标处，画任意一条线
void draw_row_line(int x,int y,int n,const char type[],int color){
    set_cursor(1,1);
    set_cursor(x,y);
    for(int i=0;i<n;i++){
	if(color>=1&&color<=7){
		...
		printf(yellow"%s",type);
	    else if(color==5)
		printf(blue"%s",type);
	    else if(color==6)
		printf(purple"%s",type);
	    else if(color==7)
		printf(dark_green"%s",type);
	}
	else
	    printf(white"%s",type);
    }
...
*/
void draw_col_line(int x,int y,int n,const char type[],int color){
    set_cursor(1,1);
    set_cursor(x,y);
//    save_cursor();
    for(int i=0;i<n;i++){
...
//定坐标显示字符串
void set_point_ch(int x,int y,char *s){
    set_cursor(1,1);
    set_cursor(x,y);
    printf(blue"%s",s);
}
