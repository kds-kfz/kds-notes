...
char check_person(Mysql_t *m,Custom *p){
    char check_buf[1024];
    //按帐号查找，有相同的则不能注册
    sprintf(check_buf,"select count(*) from person where account='%s';",p->account);
...
char check_film(Mysql_t *m,Film *f){
    char check_buf[1024];
    //按帐号查找，有相同的则不能注册
    sprintf(check_buf,"select count(*) from film where name='%s';",
	    f->film.cinema_name);
    Mysql_query(m,check_buf);
...
char check_film_by_num(Mysql_t *m,Film *f){
    char check_buf[1024];
    //按帐号查找，有相同的则不能注册
    sprintf(check_buf,"select count(*) from film where number='%d';",
	    f->film.cinema_number);
    Mysql_query(m,check_buf);
...
char check_movie_by_name(Mysql_t *m,Movie *m2){
    char check_buf[1024];
    sprintf(check_buf,"select count(*) from movie where m_name='%s';",
	    m2->movie_name);
    Mysql_query(m,check_buf);
...
char check_movie_by_num(Mysql_t *m,Movie *m2){
    char check_buf[1024];
    sprintf(check_buf,"select count(*) from movie where m_number=%d;",
	    m2->movie_number);
    Mysql_query(m,check_buf);
...
void get_flim_news(Mysql_t *m,Film *f){
    char check_buf[1024];
    sprintf(check_buf,"select *from film where number=%d",f->film.cinema_number);
    Mysql_query(m,check_buf);

    m->result=mysql_store_result(m->connect);
    m->row=mysql_fetch_row(m->result);
    f->film.cinema_number=atoi(m->row[0]);
    strcpy(f->film.cinema_name,m->row[1]);
    f->film_num=atoi(m->row[2]);
}
void get_movie_news(Mysql_t *m,Movie *m2){
    char check_buf[1024];
    sprintf(check_buf,"select *from movie where m_number=%d;",
	    m2->movie_number);
    Mysql_query(m,check_buf);
    ...
    strcpy(m2->movie_name,m->row[1]);
    strcpy(m2->c_name,m->row[3]);
    strcpy(m2->play_time,m->row[4]);
    strcpy(m2->show_time,m->row[5]);

    m2->movie_number=atoi(m->row[0]);
    m2->c_number=atoi(m->row[2]);
    m2->movie_price=atoi(m->row[6]);
    m2->ticket_count=atoi(m->row[7]);
    m2->play_yard=atoi(m->row[8]);
}

void insert_all_data(Mysql_t *m){
    Custom c1={"1001","lisi","key123","24","男","12345678901","深大","999","client"};
    Custom c2={"1002","keke","key123","24","男","12345678901","深大","999","client"};
    Custom c3={"1003","mimi","key123","24","男","12345678901","深大","999","client"};
    Custom c4={"1004","dada","key123","24","男","12345678901","深大","999","admin"};
    
    Movie m1={4001,"变形金刚1",3001,"大地影院","120分钟","2017-12-23",79,40,1};
    Movie m2={4002,"变形金刚2",3001,"大地影院","120分钟","2017-12-23",89,40,1};
    Movie m3={4003,"变形金刚3",3001,"大地影院","120分钟","2017-12-23",77,40,2};
    Movie m4={4004,"变形金刚4",3001,"大地影院","120分钟","2017-12-23",92,40,2};
    Movie m5={4005,"东成西就1",3002,"乐巢影院","120分钟","2017-12-23",67,40,3};
    Movie m6={4006,"东成西就2",3002,"乐巢影院","120分钟","2017-12-23",85,40,3};
    Movie m7={4007,"神奇四侠1",3002,"乐巢影院","120分钟","2017-12-23",93,40,4};
    Movie m8={4008,"神奇四侠2",3002,"乐巢影院","120分钟","2017-12-23",74,40,4};
